...
for helper in remote_remediate_v3.py verify_publication.py exercise_links.py; do
...
python .remediation-runtime/exercise_links.py --url http://127.0.0.1:8765/ | tee evidence/local-link-console.txt
cleanup_server
trap on_exit EXIT

allowed='^(README\.md|CUSTOMER-GUIDE\.md|SUPPORT\.md|CONTRIBUTING\.md|SECURITY\.md|documentation-manifest\.json|docs/|verification/|\.github/workflows/deploy-pages\.yml)$'
...
python .remediation-runtime/exercise_links.py --url "$pages_url" | tee evidence/live-link-console.txt
