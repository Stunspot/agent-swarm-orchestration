...
EXPECTED BEHAVIORS
0. Preserves the completed schema return as agent-reported pending root observation.
1. Classifies the failure as wrong-target context, observes the authoritative repository root, changes only that packet field, and reassigns only migration history.
2. Does not repeat the broken packet or invent the correct path.

ACCEPTABLE VARIATION
- Has the root perform the migration inspection after observing the correct root.

MATERIAL FAILURE SIGNALS
0. Discards completed schema work or upgrades it to verified evidence.
1. Restarts the whole swarm.
2. Guesses a repository path or asks the user for one before using the stated authoritative observation step.
